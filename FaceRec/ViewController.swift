...
class ViewController: UIViewController {
    ...
    private var rekognition: Rekognition!
...
extension ViewController {
    ...
    func handleFaces(request: VNRequest, error: Error?) {
        DispatchQueue.main.async {
            ...
            if results.count>0 {
                self.rekognition.searchFaces(by: self.testView.image!)
            } else {
                DispatchQueue.main.async {
...
extension ViewController: RekognitionDelegate{
    // MARK: - RekognitionDelegate
    func getFaceInfo(_ info: AWSRekognitionFace!) {
        guard let eid = info.externalImageId else {
            DispatchQueue.main.async {
                self.nameLabel.isHidden = true
            }
            return}
        self.rekognition.getFaceInfo(eid) { (finfo) in
            guard let faceinfo = finfo else { return }
            let name = faceinfo["name"] as! String
            let title = faceinfo["title"] as! String
            DispatchQueue.main.async {
                self.nameLabel.text = name+" "+title+" 您好";
...
extension ViewController: AVCaptureVideoDataOutputSampleBufferDelegate{
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        let attachments = CMCopyDictionaryOfAttachments(kCFAllocatorDefault, sampleBuffer, kCMAttachmentMode_ShouldNotPropagate)
        ...
        let uiImage:UIImage? = UIImage.init(cgImage: imageRef!, scale: 0.6, orientation: UIImageOrientation.leftMirrored)
        if let myImage = uiImage {
            DispatchQueue.main.async {
                self.testView.image = myImage
            }
        }
